...
class OpenaiController {
    ...
    async chat(req, res) {
        try {
            ...
            await this.deductUserCredits(userId, totalTokens);
            res.write("data: [DONE]\n\n");
            await Message.create({
                content: fullContent,
                chatId: chatId,
    ...
    async deductUserCredits(userId, total_tokens) {
        if (!userId) {
        ...
        try {
            ...
            // Deduct credits
            await user.save();
            console.log(`Successfully deducted ${total_tokens} credits. User now has ${user.credits} credits.`);
            return {
                success: true,
