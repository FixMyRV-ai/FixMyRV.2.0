 ...
 */
const logTwilioWebhook = async (messageSid, accountSid, fromNumber, toNumber, messageBody, numMedia = 0, webhookUrl, isTestMessage = false, status = 'received', errorMessage, processingTimeMs, rawPayload) => {
    try {
...
export const receiveSmsWebhook = async (req, res) => {
    ...
    try {
        ...
        if (!twilioSettings) {
            ...
            await logTwilioWebhook(MessageSid, AccountSid, From, To, Body, NumMedia ? parseInt(NumMedia) : 0, webhookUrl, false, 'error', errorMsg, Date.now() - startTime, req.body);
            res.status(500).json({
                success: false,
                message: errorMsg,
        ...
        if (AccountSid !== twilioSettings.accountSid) {
            ...
            // Log the failed attempt
            await logTwilioWebhook(MessageSid, AccountSid, From, To, Body, NumMedia ? parseInt(NumMedia) : 0, webhookUrl, false, 'failed', errorMsg, Date.now() - startTime, req.body);
            res.status(403).json({
                success: false,
        ...
        if (twilioSignature && process.env.NODE_ENV === 'production') {
            const isValidSignature = validateTwilioSignature(twilioSettings.authToken, twilioSignature, webhookUrl, req.body);
            if (!isValidSignature) {
        ...
        await logTwilioWebhook(MessageSid, AccountSid, From, To, Body, NumMedia ? parseInt(NumMedia) : 0, webhookUrl, false, 'processed', undefined, processingTime, req.body);
        ...
        try {
            await logTwilioWebhook(req.body?.MessageSid || 'UNKNOWN', req.body?.AccountSid || 'UNKNOWN', req.body?.From || 'UNKNOWN', req.body?.To || 'UNKNOWN', req.body?.Body || '', req.body?.NumMedia ? parseInt(req.body.NumMedia) : 0, `${req.protocol}://${req.get('host')}${req.originalUrl}`, false, 'error', errorMessage, processingTime, req.body);
        }
        catch (logError) {
 ...
 */
export const testSmsWebhook = async (req, res) => {
    const startTime = Date.now();
    try {
        ...
        await logTwilioWebhook(messageData.MessageSid, messageData.AccountSid, messageData.From, messageData.To, messageData.Body, parseInt(messageData.NumMedia) || 0, webhookUrl, true, // This is a test message
        'processed', undefined, processingTime, messageData);
        res.status(200).json({
            success: true,
        ...
        try {
            await logTwilioWebhook('TEST_ERROR', 'TEST_ACCOUNT', req.body?.From || req.body?.from || 'UNKNOWN', req.body?.To || req.body?.to || 'UNKNOWN', req.body?.Body || req.body?.body || 'Test message failed', 0, `${req.protocol}://${req.get('host')}/api/v1/twilio/test/sms`, true, 'error', errorMessage, processingTime, req.body);
        }
        catch (logError) {
