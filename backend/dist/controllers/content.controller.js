...
class ContentController {
    ...
    constructor() {
        ...
        this.CREDENTIALS_FILE = path.join(__dirname, "../config/google-credentials.json");
            ...
            try {
                ...
                const hasProtection = await page.evaluate(() => {
                    return (document.querySelector('input[name="cf-turnstile-response"]') !==
                        ...
                        document.body.textContent?.includes("Enable JavaScript and cookies to continue") ||
        ...
        this.processDocumentAndGenerateVectorStore = async (docs, metadata, sourceInDB) => {
        ...
        this.uploadPDF = async (req, res) => {
            ...
            try {
                ...
                try {
                    ...
                    res.status(400).json({
                        error: "The PDF appears to be empty or contains no extractable text content. Please ensure the PDF contains selectable text.",
                    });
                    return;
                }
                ...
                const vectorizationResponse = await this.processDocumentAndGenerateVectorStore(docs, metadata, fileInDB);
        ...
        this.getSources = async (req, res) => {
            try {
                ...
                const { rows: sources, count: totalItems } = await SourceContent.findAndCountAll({
                    offset,
                    limit: limitNumber,
                    order: [["id", "DESC"]],
                });
        ...
        this.bulkDeleteSources = async (req, res) => {
            try {
        ...
        this.authorizeGoogleDrive = async (req, res) => {
            try {
        ...
        this.getGoogleDriveFiles = async (req, res) => {
            try {
                if (!this.oauth2Client) {
                    res
                        .status(500)
                ...
                const response = await drive.files.list({
                    q: query,
                    fields: "files(id, name, mimeType, owners, shared, createdTime, modifiedTime, size, webViewLink, parents)",
                    pageSize: 100,
                ...
                const uploadedFileIds = new Set(uploadedFiles.map((f) => f.googleDriveFileId));
                const processedItems = response.data.files?.map((item) => {
                    ...
                    return {
                        ...
                        isFolder,
                        isUploaded: !isFolder && item.id ? uploadedFileIds.has(item.id) : false,
                    };
                });
                ...
                if (error instanceof Error &&
                    ...
                    res.status(403).json({
                        error: "Insufficient permissions to access Google Drive. Please check service account permissions.",
                    });
                }
        ...
        this.uploadFromGoogleDrive = async (req, res) => {
            try {
                ...
                const vectorizationResponse = await this.processDocumentAndGenerateVectorStore(docs, {
                    ...metadata,
        ...
        this.bulkUploadFromGoogleDrive = async (req, res) => {
            try {
                        ...
                        try {
                            ...
                            try {
                                ...
                                return {
                                    fileId,
                                    status: "error",
                                    message: "The PDF appears to be empty or contains no extractable text content",
                                };
                            }
                            // Only proceed with database operations if we have valid content
                            if (!docs || docs.length === 0) {
                            ...
                            await this.processDocumentAndGenerateVectorStore(docs, {
                                ...metadata,
                                originalSource: "Google Drive",
                                originalFileName: fileName,
                                googleDriveFileId: fileId,
        ...
        try {
            ...
            if (fs.existsSync(this.CREDENTIALS_FILE)) {
                ...
                this.oauth2Client = new google.auth.JWT(credentials.client_email, undefined, credentials.private_key, this.SCOPES);
                ...
                console.warn("Google Drive service account credentials file not found at:", this.CREDENTIALS_FILE);
    ...
    async processScrapedContent(content, url) {
        try {
            // Validate content
            if (!content ||
            ...
            await this.processDocumentAndGenerateVectorStore(docs, document.metadata, URLinDB);
        }
        catch (error) {
    ...
    async extractPageContent(page, url) {
        try {
            ...
            if (pageContent.includes("Enable JavaScript and cookies to continue") ||
    ...
    async extractMetadata(page) {
        const metadata = {};
        try {
            const metaTags = await page.$$eval("meta", (tags) => tags
                .map((tag) => ({
    ...
    async extractImages(page) {
        return await page.$$eval("img", (imgs) => imgs
            .map((img) => img.src)
    ...
    async extractContentWithFastAPI(filePath) {
        try {
            ...
            if (!text || text.trim().length === 0) {
                throw new Error("No content could be extracted from the PDF using FastAPI");
            }
            return {
                content: text,
                metadata: {
    ...
    async generateDocumentsFromPDF(filePath) {
        ...
        try {
            ...
            if (!singleDoc || singleDoc.length === 0 || !singleDoc[0]?.pageContent) {
                ...
                console.log("Regular PDF loader failed, attempting to extract content with FastAPI...");
            ...
            if (!singleDoc[0].pageContent.trim()) {
                ...
                console.log("PDF content is empty, attempting to extract content with FastAPI...");
                const fastAPIResult = await this.extractContentWithFastAPI(filePath);
                const textSplitter = new RecursiveCharacterTextSplitter({
                    chunkSize: 1000,
                    chunkOverlap: 200,
