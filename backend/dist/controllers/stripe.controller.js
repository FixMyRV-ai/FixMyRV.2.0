...
class StripeController {
    ...
    static async handleWebhook(req, res) {
        ...
        try {
            ...
            eventObject = stripe.webhooks.constructEvent(rawBody, signature, process.env.STRIPE_WEBHOOK_SECRET);
        }
        catch (err) {
    ...
    static async handleCheckoutSessionCompletedWebhook(event) {
        try {
    ...
    static async handleInvoicePaymentSucceededWebhook(event) {
        try {
            ...
            if (user.sessionId) {
                user.sessionId = null;
                await user.save();
                console.log(`Successfully processed invoice payment for user ${user.get("id")}`);
                return;
            }
            const creditsToAdd = parseInt(product.metadata?.credits);
            ...
            // Save user changes
            await user.save();
            console.log(`User ${user.get("id")} upgraded to pro and added ${creditsToAdd} credits after schedule completion`);
        }
        catch (error) {
            console.error("Error processing invoice payment succeeded webhook:", error);
        }
    }
}
